...
export default function CollegesPage() {
  ...
  const fetchColleges = useCallback(async () => {
    ...
    const data = await getColleges(params).catch(() => ({ colleges: [], total: 0, page: 1, totalPages: 1 }));
  ...
  useEffect(() => { setPage(1); }, [search, selectedState, selectedCourse, selectedFee]);
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 mb-8">
        <div className="flex gap-3 flex-wrap">
          ...
          {(search || selectedState || selectedCourse || selectedFee !== 0) && (
            <button
              onClick={() => { setSearch(""); setSelectedState(""); setSelectedCourse(""); setSelectedFee(0); }}
              className="flex items-center gap-1.5 px-4 py-2.5 text-slate-500 hover:text-red-500 text-sm border border-slate-200 rounded-xl hover:border-red-200 transition-colors"
            >
              <X size={14} /> Clear
            </button>
          )}
      ...
      {compareList.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white rounded-2xl shadow-2xl px-6 py-4 flex items-center gap-4">
          ...
          <button
            ...
            className="bg-blue-500 hover:bg-blue-400 disabled:opacity-40 text-white font-semibold px-5 py-2 rounded-xl text-sm transition-colors"
          >
            Compare →
          </button>
        </div>
